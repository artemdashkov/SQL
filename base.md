...
- [Запросы](#Запросы)
    - [Запросы на создание таблицы](#Запросы-на-создание-таблицы)
        - [CREATE](#CREATE)
    - [Запросы на добавление](#Запросы-на-добавление)
        - [INSERT](#INSERT)
...
- [Связи](#Связи)
    - [information_schema](#information_schema)
        ...
        - [information_schema.schemata](#information_schemata)
        - [information_schema.table_constraints](#information_schematable_constraints)
- [Подзапросы](#Подзапросы)
    - [Виды вложенных запросов](#Виды-вложенных-запросов)
    - [EXISTS](#EXISTS)
...
7. OFFSET: пропустить несколько строк из отсортированного набора результатов
8. LIMIT: вывести ограниченное количество строк
...
см.также другие [типы данных в SQL](#типы-данных-в-sql)


### Создание таблиц на основе уже существующих таблиц
...
## COUNT
Если в столбцах есть значения Null, то
...
### ROUND
```sql
-- округлит до двух знаков после запятой
...
таблица, в которой хранится информация о первичных и внешних ключах в таблицах. С помощью такого запроса мы можем увидеть все ключи таблицы shipment нашего датасета.
```sql
select 
    c.table_schema,
    c.table_name,
...
order by 
  1
  """
Все части первого SELECT выглядят как обычно — единственное нововведение состоит в том, что в разделе WHERE после EXISTS идет другой запрос. Внутри этого запроса неважен список полей: в нем будет считаться только количество строк в результате, с учетом условия s.city_id = c.city_id. Вы наверняка уже поняли, что получится: в таблице доставок будет проверяться наличие доставок в конкретный город.

Условие, как обычно, необязательно должно быть равенством, главное — чтобы его результат имел логический тип. Функция проверки условия выполняется для каждой строки и, как и в случае с INNER JOIN, остаются только те города, в которых были доставки. Из них мы и выбираем уникальные названия штатов.
  """
```
